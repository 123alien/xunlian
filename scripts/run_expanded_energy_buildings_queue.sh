...
OUT="results/phase2_paper/expanded"
...
"$PY" src/phase2.py \
  ...
  --skip-baselines \
  --skip-injection \
  > "$LOGDIR/01_neural_lstm_dlinear_patchtst.log" 2>&1
...
"$PY" run_compare_forecasting_methods.py \
...
"$PY" scripts/source_target_similarity_analysis.py \
  --data "$DATA" \
  --building-manifest "$MANIFEST" \
